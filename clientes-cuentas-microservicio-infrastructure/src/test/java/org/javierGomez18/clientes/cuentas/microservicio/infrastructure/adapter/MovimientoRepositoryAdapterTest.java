...
class MovimientoRepositoryAdapterTest {
  ...
  @Mock MovimientoJpaRepository movimientoJpaRepository;

  @Mock MovimientoEntityMapper movimientoEntityMapper;

  @InjectMocks MovimientoRepositoryAdapter adapter;
  ...
  void findByCuentaId_null_throws() {
    assertThrows(
        ...
        () ->
            adapter.findByCuentaId(
                null, PageRequest.builder().page(0).size(10).descending(false).build()));
  }
  ...
  void addMovimiento_nullCuentaId_throws() throws Exception {
    ...
    mov.setTipo(
        org.javierGomez18.clientes.cuentas.microservicio.domain.model.TipoMovimiento.INGRESO);
  ...
  void findByCuentaId_delegatesAndMaps() {
    ...
    when(movimientoJpaRepository.findByCuentaId(
            eq(cuentaId), any(org.springframework.data.domain.PageRequest.class)))
        .thenReturn(page);

    var result =
        adapter.findByCuentaId(
            cuentaId, PageRequest.builder().page(0).size(10).descending(false).build());
