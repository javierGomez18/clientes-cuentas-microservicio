    ...
    public Movimiento createMovimiento(CreateMovimientoCommand command) {
        ...
        TipoMovimiento tipoOrigen =
                tipoMovimiento == TipoMovimiento.TRANSFERENCIA
                        ...
                        : tipoMovimiento;
        CuentaBancaria cuentaOrigen = cuentaBancariaQueryRepository.findCuenta(command.cuentaIdOrigen())
        ...
        if(tipoOrigen == TipoMovimiento.RETIRADA){
            Optional.of(cuentaOrigen)
                    ...
                    .orElseThrow(() -> new MovimientoUnprocessableEntityException(
                            command.cuentaIdOrigen(),
                            importe));
        }


        Movimiento movimientoOrigen = movimientoCommandRepository.addMovimiento(Movimiento.builder()
                        ...
                        .build())
                .orElseThrow(() -> new IllegalStateException("No se pudo persistir el movimiento de origen"));

        cuentaOrigen.aplicarMovimiento(
                tipoOrigen,
                command.cantidad().floatValue()
        );
        cuentaBancariaCommandRepository.updateCuenta(cuentaOrigen);

        if (tipoMovimiento == TipoMovimiento.TRANSFERENCIA) {
            CuentaBancaria cuentaDestino = cuentaBancariaQueryRepository.findCuenta(command.cuentaIdDestino())
                    .orElseThrow(() -> new CuentaNotFoundException(command.cuentaIdDestino()));
            cuentaDestino.aplicarMovimiento(
                    TipoMovimiento.INGRESO,
                    command.cantidad().floatValue()
            );
            movimientoCommandRepository.addMovimiento(Movimiento.builder()
                    .fechaOperacion(LocalDate.now())
                    ...
                    .build())
                    .orElseThrow(() -> new IllegalStateException("No se pudo persistir el movimiento de destino"));
