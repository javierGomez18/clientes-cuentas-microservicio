...
class CreateMovimientoServiceTest {
  ...
  @Mock private CuentaBancariaCommandRepository cuentaBancariaCommandRepository;

  @Mock private CuentaBancariaQueryRepository cuentaBancariaQueryRepository;

  @Mock private MovimientoCommandRepository movimientoCommandRepository;

  @InjectMocks private CreateMovimientoService service;
  ...
  void shouldCreateRetirada_whenSufficientFunds() {
    ...
    when(movimientoCommandRepository.addMovimiento(any()))
        .thenReturn(Optional.of(movimientoOrigin));

    var cmd =
        new CreateMovimientoUseCase.CreateMovimientoCommand(1L, null, "RETIRADA", 100.0, "retiro");
  ...
  void shouldCreateTransfer_betweenAccounts() {
    ...
    when(movimientoCommandRepository.addMovimiento(any()))
        .thenReturn(Optional.of(movOrigin), Optional.of(movDest));

    var cmd =
        new CreateMovimientoUseCase.CreateMovimientoCommand(1L, 2L, "TRANSFERENCIA", 100.0, "tran");
